## Supplied-city traffic recovery diagnosis (2026-09-17, 13:11 export)

User asks to load/debug traffic and find existing-tool solutions or gameplay refinements. Isolated current-model replay of 418-road/74-building/72-trip export finds northern bus trapped by Divert at (2,-2), plus recurrent uncontrolled crossings. Tested recovery: pause, add balanced Lights at (10,10), (4,11), (12,18), (17,18), reopen (2,-2), resume; $300. Three minutes: 221 completed journeys/zero new crashes versus unchanged 177/four. Five minutes with reload at60s:371 journeys,16 bus-rider returns,zero new crashes/active incidents/holds>10s; all original trip IDs gone by240s. Original western crash clears in32.15s with edits (33.125s unchanged). Bus Inspect misleadingly says endpoint with hold0; clearer departure-blockage/Divert consequences recommended, not implemented. [Evidence, map and optional edited export](docs/traffic-recovery/README.md). No runtime changes, active-save edits, publication or performance benchmark.

## Occupancy-read reuse delivered locally (2026-09-17)

User selects repeated occupancy-grid work. Profiling identifies emergency yielding as90.7% of sampled builds. Individual yield queries now lazily share one fresh grid; conflict scans reuse their grid only until a crash, then return to fresh reads; batched diagnostics reuse read-only occupancy. Movement queries still see current sequential positions; no grid reuse across vehicle movement or reservation changes. Supplied-town20-second builds41,537→12,373 (70.2% fewer), retarget calls unchanged. Isolated model median1,019.39→647.85ms per5 simulated seconds (36.4% reduction, not FPS).60 supplied-town checkpoints,70 edit checks and9,600 emergency/intersection per-tick comparisons including reload match. Build and new read/freshness tests pass; full suite73/75 retains four identical baseline Flow/IntersectionSafety assertions. [Patch, profile and evidence](docs/performance-review/occupancy-reads/README.md). No active-save edits or publication.

## Failed-route reuse delivered locally (2026-09-17)

User authorizes caching failed routing while preserving immediate reactions and live occupancy. Topology-stable replanning shares graph validation; civilian/bus waiting states reuse only committed no-route results with exact topology, building, bus-route/stop-index, incident and trip-input validation. Services/patrols and route candidates blocked by occupancy remain live. No retry timer, save-format or traffic-rule changes. Supplied-town12 stranded vehicles each query1 rather than800 times in20seconds; total retarget calls12,832→3,244. Seven isolated5-second model samples improve median35.3%; not browser FPS.60 full-state checkpoints and70 edited-state comparisons match; six new tests and build pass. Full suite72/74 retains four identical baseline Flow/IntersectionSafety assertions. [Patch, methods and results](docs/performance-review/failed-routes/README.md). No active-save edits or publication.

## HUD walking-query optimization delivered locally (2026-09-17)

User selects HUD performance work. Walking queries now reject impossible distance bounds before graph allocation; the synchronous scene report shares a lazy sidewalk map and detached unblocked path results, discarding all cache state on exit/exception. No persistent invalidation, refresh-cadence, traffic, save or visual changes. Supplied-town isolated report calculations fall from median41.44ms to3.23ms (92.2%); this is Node CPU work, not measured browser FPS or the entire UI callback.4,800 path comparisons,20 full-state/report checkpoints and3 edit comparisons match the immediate baseline. Scope/ownership/blocked-set tests and build pass; full suite71/73 retains known Flow/IntersectionSafety failures. [Implementation, incremental patch and evidence](docs/performance-review/hud-walking/README.md). No active-save edits or publication. Failed vehicle-route retries remain separate.

## Supplied-town traffic recovery correction (2026-09-17)

User authorizes traffic logic fixes and reports service vehicles waiting at lights. Blocked mandatory merges now accumulate wait/hold and expose their blocker; service vehicles preflight route admission before backing up for an obstruction, removing the supplied police reverse/creep loop. Red-light bypass already applies to active responses and is verified for police/EMS/fire; returning crews/patrols retain normal controls. Inspector distinguishes junction occupancy from red-light waiting. Four supplied-town regressions, focused emergency/lane tests and build pass; full suite70/72 retains the identical four baseline Flow/IntersectionSafety failures.60-second replay has no new crashes and remains save-valid, but the two police still face actual lane blockers and all three incidents remain active. Repeated failed routing/HUD work remains separate; no FPS improvement claim, active-save edit or publication. [Implementation and replay](docs/performance-review/supplied-town-hitch/traffic-fix/README.md).

## Supplied congestion/hitch diagnosis (2026-09-17)

User supplied a 414-road/72-building/70-trip town, asked whether stuck traffic creates repeated work, and suggested updating jam logic. Isolated profiling finds autosave ≤1.9ms, but half-second HUD/mission reports up to136ms with ~1,086 walking fallback queries per sampled report. Twelve no-route vehicles each retry40times/second;20seconds gives12,815 retarget calls and14,631 occupancy-grid constructions. Police34374 has a blocked required merge whose early return leaves hold at0; a temporary wait-accounting correction does not clear the jam. Police34934 shows repeated reverse/creep motion. [Evidence and bounded follow-ups](docs/performance-review/supplied-town-hitch/README.md). No new shipped runtime fix, active-save edit or publication; existing optimizations remain. Reduce repeated safe read/planning work before arbitrary retry delays; physical responder/occupancy fixes need separate focused validation. Temporary test browser/server stopped.

## Local car-motion report under investigation (2026-09-17)

Latest user comparison: with both optimizations stashed, the hitch remains in the original code. User requests restoration; changes restored and stash retained. Their active town is larger than the historical supplied fixture, a possible unmeasured factor. No optimization regression established and no additional runtime fix requested.

User reports slight stop-motion cars locally after the performance work, with timing uncertain. Latest clarification: smooth motion for 2–3 seconds, then a brief hang; investigate periodic work, not continuous tick stepping. Existing 2-second autosave synchronously serializes/writes localStorage and is a timing candidate, not a confirmed cause; no new timer was added. Original/current movement model, renderer car-position function and ticker setup are unchanged; 540 fixed-frame full-state comparisons match. Both versions have inherited 40Hz unsmoothed simulation positions. Isolated software-browser samples show no clear new pacing regression but cannot exclude the user's high-refresh symptom. Test browser/servers stopped; clean reload without local test workload needs user confirmation. No smoothing change, rollback or runtime fix made. [Investigation and limits](docs/performance-review/motion-check/README.md). Do not treat functional equality as proof of visual smoothness or silently change traffic cadence to address it.

## Routing-snapshot performance pass (2026-09-17)

User authorizes the next isolated optimization: remove unused routing-snapshot work. `RoutingSnapshot.revision` had no production readers and is removed, along with its sorted/serialized signatures. Response snapshots now skip the control-delay scan whose results were discarded. Ordinary estimates, copied snapshot data, weighted route choices, responder admission, simulation cadence and saves remain unchanged; no global mutation counter. Before/after snapshots, routes and full city states match. Snapshot construction improves; full-model timings are mixed and are not an FPS claim. [Implementation, repeat measurements and checks](docs/performance-review/routing-snapshot/README.md). Build/focused checks pass; full suite remains 69/71 files with identical four Flow/IntersectionSafety baseline failures. Existing road-warning-cache work is preserved. No publication or active-save edits.

## Incremental road-warning performance pass (2026-09-17)

User requests evaluating five optimization proposals against current source and implementing improvements one at a time. First local change caches residential road-access warning reasons using the validated graph identity and exact building/entrance values; renderer and HUD reuse results until access changes. No saved flags or global mutation counter. Conditional label styles and part of road-string removal already existed. Destination flow fields and lane lists remain unimplemented architectural proposals; weighted routes and sequential occupancy must be preserved. Warm warning-query work on the historical 408-road town falls 81.5% (about 0.114ms/read), not a measured FPS gain. [Assessment, implementation, evidence and limits](docs/performance-review/access-cache/README.md). Build and focused checks pass; 180 full-state checkpoints match. Full suite 69/71 files; four Flow/IntersectionSafety assertions reproduce on clean starting HEAD. No publication or active-save edits.

## Beginner road-puzzle review: Levels 1–10 (2026-09-14)

User requests a retroactive, easy-update pass: teach road connections first (cars from A to B), then increase complexity and the available strategies. Preserve the jam scope and defer larger redesigns. Levels 1–3 remain connection problems; 5 remains a safe crossing; 6 accepts any safe working control/layout; 8 keeps the inherited eastbound main street but accepts any legal return; 9 protects the center island but accepts safe alternatives to a directed ring; 10 accepts any bridge carrying actual shopping trips, removing the arbitrary four-new-wide-section requirement. Old metadata and permanent awards remain. Briefings and goals describe situations/outcomes without click recipes.

Level 4's existing Stops requirement is retained and stated as a stop-controlled crossing outcome: removing it makes the starting town win untouched, so its map needs a later redesign. Level 7's sustained-flow thresholds and Level 2's 45-second timer are preserved, with the latter still visible during play. [Review, validation and post-jam caveats](docs/challenges/beginner-puzzle-review/README.md). No map/budget changes, revisions/resets, active-save edits, publication or performance tests.

## Jam pass on Levels 20–25 (2026-09-14)

User requests checking the final six missions against the road-puzzle intent, while explicitly leaving deeper design work until after the jam and prioritizing participation over perfection. The bounded review keeps the maps/budgets of 20 and 22–25, verifies two materially different legal solutions to every level, and replaces solution instructions with situations and outcome-based goals. Level 21's revision 4 removes the mandatory tool/restoration sequence as recorded below. Levels 24/25 have separate Show crash/Show fire navigation. Narrow layouts keep tools in a horizontal shelf so the current goal remains visible. [Review, evidence and post-jam limitations](docs/challenges/jam-puzzle-review/README.md). Local only; no publication, active-save edits, economy rebalance or performance tests.

## Missions are road puzzles, not tutorials (2026-09-14)

User explicitly corrects mission design: the fun is solving simple road challenges, not following tool instructions. Present a readable road problem and outcome; let the player discover the solution. A crash that police cannot reach because of one-way roads is the chosen example. Avoid prescribed tool sequences, coordinate instructions, solution spoilers, or requiring players to undo a working design. Basic tool controls and optional location focus remain usable. This supersedes earlier instruction-following mission design where it conflicts, while preserving the specifically requested Level 19 preplaced-Divert/reopening lesson.

Level 21 (`temporary-two-way`) revision 4 now asks for legal police access, actual crash clearance and a fresh shopping return. Permanent direction changes and alternative road layouts both count. No required Divert/conversion/restoration sequence. Copy describes the situation and outcomes without giving the solution. Old revision 3 attempts archive on selection and earned awards stay earned. [Delivery and checks](docs/challenges/one-way-rescue/README.md). Other mission content has not been rewritten in this pass; use this design rule for future mission work. Local only; no publication, active-save edits or performance tests.

## Mission 19: reopen the shortcut (2026-09-14)

User rejects the confusing coordinate-based second-entrance/Divert sequence and selects a simple prebuilt lesson: a crash and Divert are already placed, cars take the long way around, then removing Divert after real police clearance opens the shorter route. Optional synchronization stars and extra construction requirements are deferred by “Just make it simple.” Level 19 (`past-the-wreck`) revision 5 supplies a left-hand detour and connected police station; only Divert is needed. Show Divert selects the tool and centers a highlighted road tile. Actual rescue, reopening, and one fresh shopping return complete the mission. Early removal explains to let police finish. Old attempts archive on selection; earned awards remain. [Implementation and checks](docs/challenges/reopen-shortcut/README.md). Local only; no publication, active-save edits or performance tests.
