...
function replan(city: City, index: RoadIndex): Set<number> {
  ...
  for (const trip of ordered) {
    ...
    if(!safe) {
      ...
      if(trip.service) {
        // A legal route may still require a turn across an occupied lane. Verify
        // admission at the centre before backing up, without changing the live trip.
        // Recheck on every tick so newly available space can unblock the response.
        const centred={...trip,progress:k},candidate={...centred};
        if(!retarget(city,candidate,trip.path[k]) || !commitTripRoute(city,index,centred,candidate))continue;
      }
...
function changeLane(index: RoadIndex, g: Grid, held: Map<number,Slot[]>, trip: Trip): boolean {
  ...
  if(slots.some(slot=>!vacant(g,slot,trip.id))) {
    if(lane===1&&!future) {
      trip.hold=round6(trip.hold+TRAFFIC_TICK);
      trip.wait=round6(trip.wait+TRAFFIC_TICK);
      return true;
    }
    return false;
  }
...
export function vehicleDebug(city:City,trip:Trip,index=roadIndex(city),g=grid(city,index).grid){
  ...
  else if(trip.trafficLane===1 && Math.abs(trip.progress-k)<1e-9 && !laneContinuation(index,trip.path,k)) {
    for(const slot of heldSlots(index,trip.path,k,slotAt(index,trip.path,k).enter!==slotAt(index,trip.path,k).exit,0))
      for(const [id,slots] of g.get(slot.tile)??[])if(id!==trip.id&&slots.some(other=>!compatible(slot,other)))blockers.add(id);
    reason=blockers.size?'Waiting for space to merge into the exit lane':'Merging into the exit lane';
  }
