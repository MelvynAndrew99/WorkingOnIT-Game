...
export type CityCommand = {type:'bus-stop';action:'inspect'|'move'|'cancel'|'close';id?:number} | {type:'transit';action:'edit'|'finish'|'cancel'|'undo'|'buy'|'sell'|'start'|'stop'|'close';busId?:number} | {type:'road-direction'; mode:'forward'|'reverse'|'two-way'|'cancel'|'undo'} | {type:'finish-tutorial'} | {type:'open-expansion'} | { type: 'tutorial'; action: TutorialAction } | { type: 'focus'; point: Point } | { type: 'connect'; point: Point } | { type: 'zoom'; factor: number } | { type: 'home' } | { type: 'expand'; direction: ExpansionDirection };
