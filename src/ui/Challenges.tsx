...
function ChallengeResult({run,next,retry,leave}:{run:ReturnType<typeof getChallengeRun>;next:(id:ChallengeId)=>void;retry:()=>void;leave:()=>void}) {
  ...
  return <dialog ref={dialog} className="level-result" aria-labelledby="result-title" onCancel={e=>{e.preventDefault();leave();}}>
    ...
    {jamRewardEarned(run)&&<p><strong>Jam campaign complete!</strong> What A Jam is yours free. Your song reward is saved with this award.</p>}
    <div className="result-score"><strong>{run.startedAt===undefined?'Saved completion':`${progress.seconds.toFixed(1)}s`}</strong><span>{run.emergency?'Emergency recovery complete':run.id==='first-road'?'Car reached the store':`${progress.served}/${progress.total} households home`}</span><span>${run.city.funds.toLocaleString()} budget left</span></div>
    ...
    <div className="challenge-ui">
      ...
      <main className="city-stage">
        <nav aria-label="Challenge map controls">
          <button aria-pressed={s.tool===null&&!s.panning&&s.movingBusStop===null&&s.transitDraft===null} onClick={()=>selectConstructionTool(null)}>Inspect</button>
      ...
      <footer className={`city-controls${run.emergency?' emergency-controls':''}`}>
        ...
        <section className="challenge-construction" aria-label="Challenge construction">
          ...
          {challengeTools(run.id).map(tool=><button key={tool} aria-pressed={s.tool===tool&&!s.panning} onClick={()=>selectConstructionTool(tool,`${names[tool]} selected. Click it again to inspect.`)}>{names[tool]}<small>{tool==='bulldoze'?'Refund':tool==='closure'||tool==='direction'?'Free':`$${constructionPriceForCity(session.city,tool)}`}</small></button>)}
          </div>
        </section>
