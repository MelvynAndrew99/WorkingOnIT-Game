...
const url=process.env.CITY_URL||'http://localhost:5181', out=fileURLToPath(new URL('.',import.meta.url));
...
try{
 for(const viewport of [{width:390,height:844},{width:1440,height:900}]){
  ...
  const expected=await page.evaluate(async({saveKey})=>{
   ...
   const c=m.createCity();c.funds=10000;c.tutorial.status='skipped';for(let x=0;x<=15;x++)m.place(c,'road',x,6);
   m.place(c,'policeStation',0,4);m.place(c,'home',4,4);m.place(c,'store',11,4);m.place(c,'road',10,5);m.place(c,'signal',10,6);m.place(c,'closure',12,6);
  ...
  assert.equal(outcome.response.phase,'working');assert.equal(outcome.civilian.phase,'waiting');assert.equal(outcome.signal,'ns');assert.ok(!outcome.response.emergencyPass);
  await page.screenshot({path:`${out}in-game-${viewport.width}-arrived-on-red.png`});
  assert.deepEqual(errors,[]);console.log(`PASS ${viewport.width}: 12 dedicated textures, paused mid-shift boot/reload, opposing-lane playthrough, passed a stranded driver and diversion, arrived during red at ${outcome.elapsed}s`);
  await context.close();
 }
