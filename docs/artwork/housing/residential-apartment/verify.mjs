 ...
 *   3. declared world offsets match the N/S = x, E/W = y contract, and keys match their entrances;
 ...
 *   6. the drawn unit count matches the residents the manifest declares (4 upgrading to 6).
 *   node docs/artwork/housing/residential-apartment/verify.mjs
 */
...
const PREFIX = 'apartment';
/** Building rows per level (top .. exclusive base row) and the block's opaque column span. */
const BAND = { 1: [20, 53], 2: [12, 53] }, COLS = [13, 51];
const ASPHALT = [0x47, 0x4e, 0x53];
/** Balcony rail rows: one per unit floor, counted to prove the 4 -> 6 upgrade is really drawn. */
const RAILS = { 1: [34, 42], 2: [26, 34, 42] };
...
for (const key of keys) {
  ...
  check(f.residents === (f.level === 1 ? 4 : 6), `${key}: residents ${f.residents}`);
  check(f.units === f.residents, `${key}: ${f.units} drawn units for ${f.residents} residents`);
}
...
// The upgrade is really drawn: one more balcony rail row, and two cream rails on every rail row.
const CREAM = [0xf0, 0xe2, 0xc6];
for (const level of [1, 2]) {
  const f = manifest.frames[`${PREFIX}_${level}_S`] ?? manifest.frames[`${PREFIX}_2_S_S_3`];
  for (const row of RAILS[level]) {
    let runs = 0, inRun = false;
    for (let x = COLS[0]; x < COLS[1]; x++) {
      const c = get(sheet, f.x + x, f.y + row);
      const cream = CREAM.every((v, i) => c[i] === v);
      if (cream && !inRun) runs++;
      inRun = cream;
    }
    check(runs === 2, `level ${level}: row ${row} has ${runs} balcony rails, expected 2`);
  }
  check(RAILS[level].length * 2 === (level === 1 ? 4 : 6), `level ${level}: rail rows do not add up to the declared residents`);
}
...
console.log(`OK: ${keys.length} apartment frames, driveways match the manifest, primaries fixed, block identical per level, 4 -> 6 units drawn.`);
