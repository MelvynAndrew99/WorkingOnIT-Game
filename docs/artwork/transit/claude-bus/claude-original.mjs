...
const SRC_CITY = '/tmp/claude-bus-art-scope/modern-city.png';
const SRC_URBAN = '/tmp/claude-bus-art-scope/urban-pack.png';
...
function drawSide(flip) {
  ...
  for (const c of [8, 29]) {
    R(c - 3, 17, 7, 3, DARK);          // wheel arch cut into the body
    R(c - 2, 17, 5, 1, TIRE);
    R(c - 3, 18, 7, 2, TIRE);
    R(c - 2, 20, 5, 1, TIRE);
    R(c - 1, 18, 3, 2, HUB);
...
function drawEnd(isRear) {
  ...
  for (const cx of [2, 17]) {
    rect(img, cx, 22, 3, 5, TIRE);
    rect(img, cx, 22, 3, 1, TIRE_L);
    rect(img, cx, 27, 3, 1, DARK);
  }
