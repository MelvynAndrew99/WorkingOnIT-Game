...
const out='/tmp/integrated-memory';mkdirSync(out,{recursive:true});
...
try{for(const width of [1440,390])for(const [tag,url] of [['before','http://127.0.0.1:5311'],['after','http://127.0.0.1:5312']]){
 ...
 await ctx.addInitScript(raw=>localStorage.setItem('city-workshop:city:v1',JSON.stringify(raw)),raw);await page.goto(url);const cdp=await ctx.newCDPSession(page);await cdp.send('HeapProfiler.enable');const samples=[];
 for(let cycle=0;cycle<4;cycle++){
  await page.locator('.title-sandbox').click();await page.locator('canvas').waitFor();await page.waitForTimeout(500);
  await page.evaluate(()=>window.__gameModules['/src/game/cityControls.ts'].cityCommand({type:'zoom',factor:.25}));await page.waitForTimeout(200);
  await page.evaluate(()=>window.__gameModules['/src/game/cityControls.ts'].cityCommand({type:'zoom',factor:4}));
 ...
 results.push({tag,width,samples});writeFileSync(`${out}/results.json`,JSON.stringify(results,null,2));console.log({tag,width,MiB:samples.map(s=>s.heap.usedSize/1048576),nodes:samples.map(s=>s.dom.nodes),listeners:samples.map(s=>s.dom.jsEventListeners)});await ctx.close();
